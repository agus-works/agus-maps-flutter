...
part of the current plugin source set. The current generated Swift source lives
under `ios/agus_maps_flutter/Sources/agus_maps_flutter/`.
...
├── Frameworks/                     # Downloaded/generated XCFramework
│   └── CoMaps.xcframework/
└── Resources/
    └── shaders_metal.metallib      # CocoaPods Metal shader resource
```

The old `ios/Classes` source layout was replaced so the same plugin-owned
Apple code can be compiled by both CocoaPods and Swift Package Manager.
...
| `ios/agus_maps_flutter/Sources/agus_maps_flutter_native/agus_maps_flutter_ios.mm` | Modified: register callback with rate limiting |
