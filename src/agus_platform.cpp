...
// Forward declaration for the shared localization function
...
extern "C" __attribute__((visibility("default"))) void AgusPlatform_SetLocale(const char* locale)
{
  if (locale && *locale)
    g_localeTag = locale;
  // Forward to the shared localization system
  agus_localization_set_locale(locale);
}
...
namespace platform {
  // NOTE: Localization functions (GetLocalizedTypeName, GetLocalizedBrandName, etc.)
  // are now provided by agus_localization.cpp for all platforms.
