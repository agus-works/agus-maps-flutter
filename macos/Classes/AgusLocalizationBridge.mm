...
//
// macOS localization bridge - provides platform::GetLocalizedXxx implementations
...
void ParseStringsFile(const std::string& path, std::unordered_map<std::string, std::string>& translations)
{
    @autoreleasepool {
...
std::string GetCurrentLocaleTag()
{
    ...
    @autoreleasepool {
        ...
        if (langCode) {
            // Convert to simple language code (e.g., "en-US" -> "en")
...
bool TryLoadTranslations(const std::string& localeTag)
{
    @autoreleasepool {
        // Try to find .strings files in the main bundle
        NSBundle* mainBundle = [NSBundle mainBundle];
        
        // Try LocalizedStrings/localeTag/Localizable.strings
        NSString* nsLocale = [NSString stringWithUTF8String:localeTag.c_str()];
        
        // Look for LocalizableTypes.strings (POI type names)
        NSString* typesPath = [mainBundle pathForResource:@"LocalizableTypes" 
                                                   ofType:@"strings"
                                              inDirectory:[NSString stringWithFormat:@"LocalizedStrings/%@", nsLocale]];
        
        // Also try the plugin bundle
        if (!typesPath) {
            NSBundle* pluginBundle = [NSBundle bundleForClass:NSClassFromString(@"AgusLocalizationBridge") ?: [NSBundle mainBundle].class];
            typesPath = [pluginBundle pathForResource:@"LocalizableTypes" 
                                               ofType:@"strings"
                                          inDirectory:[NSString stringWithFormat:@"LocalizedStrings/%@", nsLocale]];
        }
        ...
        // Look for Localizable.strings (general strings)
        NSString* stringsPath = [mainBundle pathForResource:@"Localizable" 
                                                     ofType:@"strings"
                                                inDirectory:[NSString stringWithFormat:@"LocalizedStrings/%@", nsLocale]];
        if (!stringsPath) {
            NSBundle* pluginBundle = [NSBundle bundleForClass:NSClassFromString(@"AgusLocalizationBridge") ?: [NSBundle mainBundle].class];
            stringsPath = [pluginBundle pathForResource:@"Localizable" 
                                                 ofType:@"strings"
                                            inDirectory:[NSString stringWithFormat:@"LocalizedStrings/%@", nsLocale]];
        }
...
std::string GetLocalizedTypeName(std::string const& type)
{
    ...
    // Return the type key as fallback (remove "type." prefix if present)
    if (type.substr(0, 5) == "type.")
...
std::string GetLocalizedString(std::string const& key)
{
    ...
    // Try type translations as fallback
