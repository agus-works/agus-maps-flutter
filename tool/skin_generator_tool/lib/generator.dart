...
class SkinGenerator {
  ...
  static Future<void> generate({
    ...
    required String skinName, // Used as a prefix but actually we output to its dir
    ...
    for (int j = 0; j < symbolSizes.length; ++j) {
      ...
      for (final file in files) {
        ...
        if (fileName.toLowerCase().endsWith('.svg')) {
          ...
          try {
            ...
            final widthAttr = svgElement.getAttribute('width')?.replaceAll('px', '');
            final heightAttr = svgElement.getAttribute('height')?.replaceAll('px', '');
          ...
          final double scaleY = (defaultHeight / pictureInfo.size.height) * scale;
          ...
          final ui.Image image = await recorder.endRecording().toImage(targetWidth, targetHeight);
          ...
          final ui.Image imageWithPadding = await recorder.endRecording().toImage(targetWidth, targetHeight);
          
          symbols.add(SymbolInfo(imageWithPadding, symbolID, targetWidth, targetHeight));
        }
      }
      ...
      print('Packing ${symbols.length} symbols (minWidth: $minWidth, minHeight: $minHeight) into max size $maxSize...');
      ...
      while (true) {
        ...
        for (final s in symbols) {
          ...
          if (rect == null) {
            overflow = true;
            print('Overflow packing symbol ${s.id} (${s.width}x${s.height}) into ${width}x${height}');
            break;
          }
        ...
        if (overflow) {
          ...
          if (width > maxSize) {
            ...
            height *= 2;
            if (height > maxSize) {
              throw Exception('Texture overflow, exceeded max size of $maxSize');
            }
          }
          continue;
        }
      ...
      for (final s in symbols) {
        ...
        finalCanvas.drawImage(s.image, ui.Offset(rect.x.toDouble(), rect.y.toDouble()), ui.Paint());
      }

      final ui.Image finalImage = await finalRecorder.endRecording().toImage(width, height);
      ...
      await xmlFile.writeAsString(builder.buildDocument().toXmlString(pretty: true));
