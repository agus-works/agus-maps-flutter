...
Future<void> extractTarBz2(String tarPath, String destDir) async {
  ...
  if (os == OSType.windows) {
    ...
    if (await _commandExists('7z')) {
      ...
      throw Exception(
          'Neither 7z nor tar found. Cannot extract TAR.BZ2 on Windows.');
    }
  } else {
    // Unix: use tar
    await runProcess('tar', ['-xjf', tarPath, '-C', destDir]);
...
Future<void> createTarBz2(String sourceDir, String archivePath) async {
  ...
  if (os == OSType.windows) {
    ...
    if (await _commandExists('7z')) {
      await runProcess(
          '7z', ['a', '-tbzip2', archivePath, path.join(sourceDir, '*')]);
    } else {
      throw Exception(
          '7z not found. Cannot create TAR.BZ2 on Windows without 7z.');
    }
  } else {
    // Unix: use tar
...
Future<void> createTarGz(String sourceDir, String archivePath) async {
  ...
  if (os == OSType.windows) {
    ...
    if (await _commandExists('tar')) {
      ...
      await runProcess(
          '7z', ['a', '-ttar', tempTar, path.join(sourceDir, '*')]);
      await runProcess('7z', ['a', '-tgzip', '-mx=1', archivePath, tempTar]);
      await File(tempTar).delete();
    } else {
      throw Exception(
          'Neither tar nor 7z found. Cannot create TAR.GZ on Windows.');
    }
  } else {
...
Future<void> extractTarGz(String tarPath, String destDir) async {
  ...
  if (os == OSType.windows) {
    // Windows: use tar (available in Windows 10+) or 7z
    if (await _commandExists('tar')) {
      await runProcess('tar', ['-xzf', tarPath, '-C', destDir],
          throwOnError: true);
    } else if (await _commandExists('7z')) {
      ...
      throw Exception(
          'Neither tar nor 7z found. Cannot extract TAR.GZ on Windows.');
    }
  } else {
    // Unix: use tar
    await runProcess('tar', ['-xzf', tarPath, '-C', destDir],
